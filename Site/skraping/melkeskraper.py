...
for line in F:
    recipe,submitter = recipes(line)
    recipe = str(recipe)+".json"
    if (submitter == 'bb'):
        bb_list.append(recipe)
    else:
...
with open('recipes/recipes.json', 'w', encoding="utf-8") as recipe_file:
    json.dump(bb_list_obj, recipe_file,  ensure_ascii=False)
    # print(recipe)

# Add to master list
with open('recipes/user-recipes.json', 'w', encoding="utf-8") as recipe_file:
    json.dump(user_list_obj, recipe_file,  ensure_ascii=False)
    # print(recipe)
